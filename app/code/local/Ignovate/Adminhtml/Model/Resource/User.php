 ...
 */
class Ignovate_Adminhtml_Model_Resource_User extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
     * Define main table
     *
    ...
    {
        $debug = true;
        $oldStores = $this->lookupStoreIds($user->getId());
        $newStores = (array)$user->getStores();
        if (empty($newStores)) {
            $newStores = (array)$user->getStoreId();
        }
        $table  = $this->getTable('ignovate_adminhtml/user_store');
    ...
    public function lookupStoreIds($userId)
    {
        ...
        $select  = $adapter->select()
            ->from($this->getTable('ignovate_adminhtml/user_store'), 'store_id')
