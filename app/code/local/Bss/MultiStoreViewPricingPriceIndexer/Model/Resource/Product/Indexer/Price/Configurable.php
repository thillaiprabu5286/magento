...
*/
class Bss_MultiStoreViewPricingPriceIndexer_Model_Resource_Product_Indexer_Price_Configurable extends Bss_MultiStoreViewPricingPriceIndexer_Model_Resource_Product_Indexer_Price_Default
{
    /**
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('multistoreviewpricingpriceindexer/product_price_indexer_cfg_option_aggregate_idx');
        }
        return $this->getTable('multistoreviewpricingpriceindexer/product_price_indexer_cfg_option_aggregate_tmp');
    }
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('multistoreviewpricingpriceindexer/product_price_indexer_cfg_option_idx');
        }
        return $this->getTable('multistoreviewpricingpriceindexer/product_price_indexer_cfg_option_tmp');
    }
    ...
    protected function _applyConfigurableOption()
    {
        ...
        $select = $write->select()
            ...
            ->columns(array('customer_group_id', 'website_id', 'store_id'), 'i')
            ->join(
            ...
            ->join(
                ...
                array())
            ->joinLeft(
                array('apd' => $this->getTable('multistoreviewpricingcatalog/product_super_attribute_pricing')),
                'a.product_super_attribute_id = apd.product_super_attribute_id'
                    . ' AND apd.website_id = 0 AND cp.value = apd.value_index AND apd.store_id = 0',
                array())
            ->joinLeft(
                array('apw' => $this->getTable('multistoreviewpricingcatalog/product_super_attribute_pricing')),
                'a.product_super_attribute_id = apw.product_super_attribute_id'
                    . ' AND apw.website_id = i.website_id AND cp.value = apw.value_index AND apw.store_id = i.store_id',
                array())
            ->join(
            ...
            ->group(array('l.parent_id', 'i.customer_group_id', 'i.website_id', 'i.store_id', 'l.product_id'));
        ...
        $select = $write->select()
            ->from(
                array($coaTable),
                array(
                    'parent_id', 'customer_group_id', 'website_id', 'store_id',
