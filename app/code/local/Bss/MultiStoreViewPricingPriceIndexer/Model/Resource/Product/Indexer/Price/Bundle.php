...
*/
class Bss_MultiStoreViewPricingPriceIndexer_Model_Resource_Product_Indexer_Price_Bundle extends Bss_MultiStoreViewPricingPriceIndexer_Model_Resource_Product_Indexer_Price_Default
{
	/**
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('multistoreviewpricingpriceindexer/bundle_price_indexer_idx');
        }
        return $this->getTable('multistoreviewpricingpriceindexer/bundle_price_indexer_tmp');
    }
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('multistoreviewpricingpriceindexer/bundle_selection_indexer_idx');
        }
        return $this->getTable('multistoreviewpricingpriceindexer/bundle_selection_indexer_tmp');
    }
    ...
    {
        if ($this->useIdxTable()) {
            return $this->getTable('multistoreviewpricingpriceindexer/bundle_option_indexer_idx');
        }
        return $this->getTable('multistoreviewpricingpriceindexer/bundle_option_indexer_tmp');
    }
    ...
    protected function _prepareBundlePriceByType($priceType, $entityIds = null)
    {
        ...
        ->columns('store_id', 'cs')
            ...
            ->joinLeft(
                ...
                'tp.entity_id = e.entity_id AND tp.website_id = cw.website_id AND cs.store_id = tp.store_id'
                    . ' AND tp.customer_group_id = cg.customer_group_id',
                array()
            )
            ->joinLeft(
                array('gp' => $this->_getGroupPriceIndexTable()),
                'gp.entity_id = e.entity_id AND gp.website_id = cw.website_id AND cs.store_id = gp.store_id'
    ...
    protected function _calculateBundleOptionPrice()
    {
        ...
        $select = $write->select()
            ->from(
                ...
                array('entity_id', 'customer_group_id', 'website_id', 'store_id', 'option_id')
            )
            ->group(array('entity_id', 'customer_group_id', 'website_id', 'store_id', 'option_id', 'is_required', 'group_type'))
            ->columns(array(
